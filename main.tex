...
Figure~\ref{fig:methodology_workflow} presents the overall research methodology workflow, from event log preprocessing through augmentation and consensus verification to evaluation.
...
\paragraph{Threats to Validity.} \emph{Internal validity}: LLM responses are non-deterministic; we mitigate this by using low temperatures for verification (0.1--0.3) and reporting averages over multiple runs. \emph{External validity}: Our six datasets cover healthcare, loan processing, municipal services, and government domains, but do not include logs with predominantly parallel or subprocess execution patterns. \emph{Construct validity}: Conformance fitness, while standard in process mining, may not capture all aspects of semantic validity; domain experts might assess trace validity differently. \emph{Conclusion validity}: We report paired $t$-tests for accuracy improvements; however, the number of datasets ($n = 6$) limits the statistical power of cross-dataset generalization claims.
