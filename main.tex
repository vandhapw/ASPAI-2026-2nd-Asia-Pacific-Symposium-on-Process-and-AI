...
\author{Anonymous Author(s)\\ \textit{Anonymous Institution}}
\date{}
...
We use the Sepsis event log~\cite{4tu_sepsis} from the 4TU Research Data repository, a small (1,050 cases, 16 activities, $\sim$830 variants) high-variability healthcare dataset where augmentation should have the largest impact. We selected Sepsis as the primary evaluation dataset because its small size and high variability make it the most challenging for data augmentation methods.

\subsection{Models}

\begin{table}[t]
\centering
\caption{LLM roles and models used in the framework.}
\label{tab:models}
\begin{tabular}{llp{5.5cm}}
\toprule
\textbf{Role} & \textbf{Model} & \textbf{Purpose} \\
\midrule
Generator & DeepSeek-R1 (671B) & Trace generation with chain-of-thought reasoning \\
Structural Verifier & GLM-4 & Deterministic structural validation against DFG \\
Temporal Verifier & Kimi-K2 (1T ctx) & Full-sequence temporal ordering validation \\
\bottomrule
\end{tabular}
\end{table}

All models are accessed via Ollama Pro cloud API. Generation temperature: 0.7; verification temperature: 0.1.
...
For downstream evaluation, we train an LSTM with prefix-based encoding~\cite{teinemaa2019outcome} using a standard architecture (2 layers, 128 hidden units, cross-entropy loss). We use temporal train/test splitting with data leakage prevention following~\cite{weytjens2024benchmark}.
